...
marking. Each configured type (by default the truly rare, high-value ones:
woodland mansion, stronghold, ancient city, jungle/desert pyramid, pillager
outpost, bastion remnant, end city) has its footprint regenerated with the same
two-phase core, skipping any structure a player is currently inside.

The **End dragon** is renewed too. Its central arena is force-loaded by the fight
and can't be chunk-regenerated in place, so on each structure-reset cycle the
fight's saved state is reset (via NMS) so a fresh dragon *can* spawn again. The
End terrain itself — including the central island, the old exit portal and end
gateways — is regenerated by the ordinary age-based sweep, exactly like every
other world (no special-case regen). Because the fight re-derives "previously
killed" from whether an exit portal / gateway still exists, the dragon actually
respawns on the next entry after that sweep has cleared them. So the end-game
journey (find the stronghold → activate the portal → beat the dragon → loot an
end city for elytra) renews, with only the fight-state reset as an End special case.
...
Resident wilderness sweep: `enabled`, `worlds` (list of name + region-dir),
`min-age-days` (default 90), `min-age-seconds` (sub-day override; -1 = off),
...
Structure reset (`structure-reset` section): `enabled`, `interval-hours`
(default 6, wall-clock aligned), `rescan-hours` (registry refresh), `footprint-margin-chunks`,
`types` (which structures to reset), and `end-central` (the End dragon:
`world`, `respawn-dragon`).
...
- Deleting chunk data bypasses block-logging plugins (e.g. CoreProtect) for the
  regenerated area.
- Structure reset enumerates structures by reading `structures.starts` from the
  region files (no chunk generation needed). Loot chests refill on reset; their
  contents are deterministic per seed + position, which is fine for a
  daily-farmable structure.
